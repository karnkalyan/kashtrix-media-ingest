...
import { FiArchive, FiCalendar, FiDownload, FiFilm, FiGrid, FiList, FiPlay, FiRefreshCw, FiSearch, FiTrash2, FiX } from 'react-icons/fi';
...
const RecordingLibrary: React.FC<Props> = ({ realtimeRecordings, settings, deleteRecording }) => {
  ...
  const mediaBase = `http://${window.location.hostname}:${settings.mediaPort}`;

  return <div className="min-w-0 space-y-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div><h2 className="flex items-center gap-2 text-xl font-bold text-[var(--text-primary)]"><FiArchive className="text-[var(--primary)]" />Recording Library</h2><p className="mt-1 text-sm text-[var(--text-secondary)]">Search, inspect, preview and manage every television recording.</p></div>
      <div className="flex items-center gap-2"><button onClick={loadAll} disabled={loading} className="inline-flex h-10 items-center gap-2 rounded-xl border border-[var(--border)] bg-white px-3 text-xs font-semibold"><FiRefreshCw className={loading ? 'animate-spin' : ''} />Refresh all</button><div className="flex rounded-xl border border-[var(--border)] bg-white p-1"><button onClick={() => setView('cards')} className={`rounded-lg p-2 ${view === 'cards' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-400'}`} title="Card view"><FiGrid /></button><button onClick={() => setView('table')} className={`rounded-lg p-2 ${view === 'table' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-400'}`} title="Table view"><FiList /></button></div></div>
    </div>
