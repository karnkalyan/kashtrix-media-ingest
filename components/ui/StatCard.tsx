...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon, trend, loading = false, className = '' }) => {
  if (loading) {
    return (
      <div className={`rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-sm)] ${className}`}>
  ...
  return (
    <div className={`group rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-sm)] transition-colors duration-150 hover:border-[var(--primary-200)] ${className}`}>
      <div className="flex items-start justify-between">
        <div className="space-y-1 min-w-0 flex-1">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)]">{label}</p>
          <p className="metric-value text-[23px] font-bold leading-tight text-[var(--text-primary)] tracking-tight">{value}</p>
          {trend && (
