...
export {
  generateHwid,
  generateBaseHardwareFingerprint,
  loadOrCreateClientId,
  loadOrCreateScopedClientId,
} from "./hwid.js";

export type LicenseStateEvent =
  | "LICENSE_REVOKED"
  | "LICENSE_SUSPENDED"
  | "LICENSE_EXPIRED"
  | "LICENSE_RESTORED"
...
export type LicenseSessionState =
  "DISCONNECTED" | "CONNECTING" | "LICENSED" | "DENIED";
...
export class LicenseClient {
  ...
  private state: LicenseSessionState = "DISCONNECTED";
  private snapshot?: EntitlementSnapshot;
  private publicKey?: CryptoKey;
  private licenseKey: string;
  private pendingEntitlements = new Map<
    string,
    {
      resolve: (v: EntitlementSnapshot) => void;
      reject: (e: Error) => void;
  ...
  requireModule(code: string): void {
    if (!this.hasModule(code)) throw new Error(`Module not licensed: ${code}`);
  }
  ...
  async connect(): Promise<EntitlementSnapshot> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const socket = connect(tlsOptions, () => {
        if (!socket.authorized) {
          fail(
            new Error(
              socket.authorizationError?.message ?? "TLS authorization failed",
            ),
          );
          socket.destroy();
          return;
    ...
    if (!this.snapshot) throw new Error("Server did not provide entitlements");
    return this.getEntitlements()!;
  }

  async refreshEntitlements(timeoutMs = 10_000): Promise<EntitlementSnapshot> {
    if (!this.socket || !this.isLicensed())
      throw new Error("License client is not connected");
    const nonce = randomUUID();
    return new Promise<EntitlementSnapshot>((resolve, reject) => {
      ...
      this.socket!.write(
        JSON.stringify({ type: "ENTITLEMENTS_REQUEST", nonce }) + "\n",
      );
    });
  }
  ...
  private async consume(
    ...
    try {
      ...
      while ((i = this.buffer.indexOf("\n")) >= 0) {
        ...
        if (msg.type === "WELCOME") {
          if (
            msg.tenantId !== this.options.tenantId ||
          ...
          if (
            ...
            void this.refreshEntitlements().catch((error) =>
              this.connectionLost(error),
            );
  ...
  private applySnapshot(
    ...
    const validFrom =
      validFromValue instanceof Date
    ...
    const snapshot: EntitlementSnapshot = {
      ...
      customerName:
        typeof details?.customerName === "string"
          ? details.customerName
          : current?.customerName,
      customerEmail:
        typeof details?.customerEmail === "string"
      ...
      appVersion:
        typeof details?.appVersion === "string"
          ? details.appVersion
          : current?.appVersion,
      platform:
  ...
  private async verifySignedEvent(event: string, signedEvent: string) {
    if (!this.publicKey) throw new Error("License public key not loaded");
  ...
  private async verifyEntitlementSnapshot(msg: any) {
    const payload = await this.verifySignedEvent(
    ...
    const signedModules = [...payload.mod]
      ...
      .sort();
    const wireEntitlements = Object.fromEntries(
      Object.entries(msg.entitlements || {}).sort(([a], [b]) =>
        a.localeCompare(b),
      ),
    );
    const signedEntitlements = Object.fromEntries(
      Object.entries(payload.ent).sort(([a], [b]) => a.localeCompare(b)),
    );
    if (
      JSON.stringify(wireModules) !== JSON.stringify(signedModules) ||
  ...
  private startHeartbeat(seconds: number) {
    ...
    const validationTimeout = Math.max(
      intervalMs * 2,
      this.options.validationTimeoutMs ?? 75_000,
    );
  ...
  private async verifyAndApply(event: LicenseStateEvent, signedEvent: string) {
    ...
    if (event === "LICENSE_KEY_REISSUED") return;
    if (
  ...
  close(intentional = true) {
    ...
    if (intentional && this.state !== "DENIED") this.state = "DISCONNECTED";
    if (socket) {
...
export class LicenseProvisioningClient {
  ...
  constructor(private readonly options: LicenseProvisioningClientOptions) {}
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        try {
          ...
          while ((index = this.buffer.indexOf("\n")) >= 0) {
            ...
            if (message.type === "PROVISIONING_WAIT") {
              ...
              if (
                typeof message.licenseKey !== "string" ||
      ...
      socket.once("error", (error) => {
        if (!settled) fail(error);
        else this.connectionLost(error);
      });
      socket.on("close", () => {
        if (!settled)
  ...
  private startHeartbeat() {
    ...
    this.heartbeat = setInterval(
      () =>
        this.socket?.write(
          JSON.stringify({ type: "PROVISION_HEARTBEAT", ts: Date.now() }) +
            "\n",
        ),
      30_000,
    );
  ...
  private connectionLost(error: Error) {
    clearInterval(this.heartbeat);
    if (!this.intentionalClose) this.options.onConnectionLost?.(error);
  }
